...
- "Open in client" from the browser: the market item you are looking at opens in the running EVE client, and a solar system named in an answer becomes a button that sets it as the autopilot destination — ESI's UI endpoints, the same channel `/market` and `/info` use in Telegram and Discord.
