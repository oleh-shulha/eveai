...
const RU = {
  ...
  marketAdminTitle: 'Снапшот рынка', marketAdminLead: 'Книга ордеров всех торговых регионов k-space из ESI. Плановый воркер обходит их каждые 5 минут по двухуровневым интервалам.',
  marketAdminNever: 'Снапшот ещё не загружался.',
  marketAdminSnapshotTime: 'Снимок', marketAdminAgeLabel: 'Возраст',
  marketAdminAge: 'Обновлён {age} мин назад', marketAdminAgeUnknown: 'Возраст снапшота неизвестен',
  marketAdminFresh: 'актуален', marketAdminStale: 'устарел',
  marketAdminRows: '{rows} ордеров · регионов: {regions}, из них устарели: {stale}',
  marketAdminRegionErrors: 'Регионов с ошибками: {count} — они сохранили прежние ордера.',
  marketAdminLastError: 'Последняя ошибка обхода: {error}',
  marketAdminWorkerOff: 'Плановый воркер выключен (MARKET_SNAPSHOT_ENABLED=false): снапшот обновляется только этой кнопкой.',
  marketAdminSweepRunning: 'Идёт обход ESI…',
  marketAdminRefresh: 'Загрузить сейчас',
  marketAdminJobCommitted: 'Готово: {rows} ордеров, обновлено регионов: {regions}.',
  marketAdminJobNotDue: 'Ничего не загружено: ESI кэширует книгу ордеров 5 минут, все регионы свежее этого окна.',
  marketAdminJobFailed: 'Обход не удался: {error}. Прежний снапшот продолжает отдаваться.',
  marketAdminForcedNote: 'Принудительная загрузка считает просроченными все регионы, игнорируя наши интервалы, но 5-минутный кэш ESI всё равно соблюдается.',
  marketAdminErrorInProgress: 'Обход уже идёт.',
  webAccessTitle: 'Доступ в интернет (Firecrawl)',
  webAccessLead: 'Даёт агенту поиск по открытому вебу и чтение отдельных страниц: патчноуты, девблоги, форумы, вики, сторонние инструменты. Приватные данные EVE в запросы не уходят.',
  webAccessEndpointLabel: 'Firecrawl сконфигурирован',
  webAccessOn: 'включён', webAccessOff: 'выключен оператором',
  webAccessDisable: 'Отключить принудительно', webAccessEnable: 'Включить обратно',
  webAccessDisabledNote: 'Инструменты web_search и fetch_web_page не выдаются модели, пока выключено. Применяется со следующего ответа.',
  webAccessEnabledNote: 'Модель получает web_search и fetch_web_page. Прочитанные страницы — недоверенные данные: агент их цитирует со ссылкой, но не выполняет как инструкции.',
  webAccessErrorInvalid: 'Недопустимое значение переключателя.',
  gateTitle: 'Приватный инстанс', gateLead: 'Этот экземпляр закрыт. Введите пароль доступа, чтобы продолжить.',
  gatePasswordLabel: 'Пароль доступа', gatePasswordPlaceholder: 'Введите пароль',
  gateSubmit: 'Войти', gateChecking: 'Проверяем…',
  gateSavedNote: 'Пароль запрашивается один раз: браузер запомнит доступ до конца срока действия.',
  gateErrorInvalid: 'Неверный пароль.',
  gateErrorTooMany: 'Слишком много попыток. Подождите и повторите позже.',
  gateLock: 'Заблокировать этот браузер',
  systemRouteHint: '{name}: проложить маршрут в клиенте EVE',
  systemRouteDone: 'Маршрут до {name} задан в клиенте.',
  systemRouteNoScope: 'Персонаж не выдал разрешение на запись маршрута. Перепривяжите его с этим доступом.',
  openInClient: 'Открыть в клиенте', openInClientBusy: 'Отправляю…',
  openInClientDone: 'Отправлено в клиент EVE.',
  openInClientNoCharacter: 'Нужен привязанный персонаж EVE.',
  openInClientNoScope: 'Персонаж не выдал разрешение на управление окнами клиента. Перепривяжите его с этим доступом.',
  openInClientUnavailable: 'Клиент EVE не отвечает — он должен быть запущен и залогинен этим персонажем.',
  openInClientRefused: 'EVE отклонил запрос к клиенту.',
  accessRestrictedNote: 'Этот экземпляр закрыт: работать в нём могут только персонажи владельца. Подключите разрешённого персонажа EVE Online.',
  ...
  perimeterChatResetHint: 'Удалить историю и начать заново в той же сессии «Периметр».',
...
const EN: Record<keyof typeof RU, string> = {
  ...
  marketAdminTitle: 'Market snapshot', marketAdminLead: 'The order book of every k-space trade region, from ESI. The scheduled worker walks them every 5 minutes on two-tier intervals.',
  marketAdminNever: 'The snapshot has never been loaded.',
  marketAdminSnapshotTime: 'Snapshot', marketAdminAgeLabel: 'Age',
  marketAdminAge: 'Updated {age} min ago', marketAdminAgeUnknown: 'Snapshot age is unknown',
  marketAdminFresh: 'current', marketAdminStale: 'stale',
  marketAdminRows: '{rows} orders · {regions} regions, {stale} stale',
  marketAdminRegionErrors: 'Regions with errors: {count} — they kept their previous orders.',
  marketAdminLastError: 'Last sweep error: {error}',
  marketAdminWorkerOff: 'The scheduled worker is off (MARKET_SNAPSHOT_ENABLED=false): this button is the only thing that refreshes the snapshot.',
  marketAdminSweepRunning: 'Walking ESI…',
  marketAdminRefresh: 'Load now',
  marketAdminJobCommitted: 'Done: {rows} orders, {regions} regions refreshed.',
  marketAdminJobNotDue: 'Nothing was loaded: ESI caches the order book for 5 minutes and every region is newer than that window.',
  marketAdminJobFailed: 'The sweep failed: {error}. The previous snapshot keeps serving.',
  marketAdminForcedNote: 'A forced load treats every region as due, ignoring our own intervals — but the 5-minute ESI cache still applies.',
  marketAdminErrorInProgress: 'A sweep is already running.',
  webAccessTitle: 'Internet access (Firecrawl)',
  webAccessLead: 'Gives the agent open-web search and the ability to read a single page: patch notes, dev blogs, forums, wikis, third-party tools. Private EVE data never travels in those requests.',
  webAccessEndpointLabel: 'Firecrawl configured',
  webAccessOn: 'on', webAccessOff: 'switched off by the operator',
  webAccessDisable: 'Force off', webAccessEnable: 'Switch back on',
  webAccessDisabledNote: 'While it is off, web_search and fetch_web_page are not offered to the model. Applies from the next reply.',
  webAccessEnabledNote: 'The model gets web_search and fetch_web_page. Fetched pages are untrusted data: the agent cites them, it does not follow them as instructions.',
  webAccessErrorInvalid: 'Invalid switch value.',
  gateTitle: 'Private instance', gateLead: 'This instance is closed. Enter the access password to continue.',
  gatePasswordLabel: 'Access password', gatePasswordPlaceholder: 'Enter the password',
  gateSubmit: 'Enter', gateChecking: 'Checking…',
  gateSavedNote: 'You are asked once: the browser keeps the access until it expires.',
  gateErrorInvalid: 'Wrong password.',
  gateErrorTooMany: 'Too many attempts. Wait a while and try again.',
  gateLock: 'Lock this browser',
  systemRouteHint: '{name}: set as the autopilot destination in the EVE client',
  systemRouteDone: 'Destination {name} set in the client.',
  systemRouteNoScope: 'This character did not grant permission to write waypoints. Link it again with that access.',
  openInClient: 'Open in client', openInClientBusy: 'Sending…',
  openInClientDone: 'Sent to the EVE client.',
  openInClientNoCharacter: 'A linked EVE character is required.',
  openInClientNoScope: 'This character did not grant permission to control client windows. Link it again with that access.',
  openInClientUnavailable: 'The EVE client is not answering — it must be running and logged in as this character.',
  openInClientRefused: 'EVE refused the request to the client.',
  accessRestrictedNote: 'This instance is restricted: only the owner characters on its allowlist can use it. Link an allowed EVE Online character.',
  ...
  perimeterChatResetHint: 'Delete the history and start again in the same Perimeter session.',
