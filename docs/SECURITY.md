...
- `WEB_ALLOWED_CHARACTER_IDS` restricts the browser app to named EVE characters: a session without one of them gets `character_not_allowed` from every `/api/web/*` route except the session bootstrap, the SSO start and the private gate, and the SSO callback refuses to attach a character outside the list to a browser login before any token or ownership row is written
- `PRIVATE_PASSWORD` locks the whole browser API behind one shared password: every `/api/web/*` route answers `unlock_required` until the visitor unlocks, while the SSO flow, `/health` and the static shell stay reachable so a login started in Telegram or the CLI still completes
- the unlock cookie is HttpOnly, SameSite=Lax, `Secure` on an https base URL, and carries only an expiry plus an HMAC over that expiry and a fingerprint of the configured password — never the password, and rotating the password invalidates every issued cookie
- the password is compared as a SHA-256 digest in constant time, and wrong attempts are budgeted per client address with a lockout that also refuses the correct password while it lasts
